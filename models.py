...
def main():
    ...
    startdate = datetime.datetime(year,2,2)
    stopdate = datetime.datetime(year,4,2)
    ...
    for day in range(start_day,stop_day):
        ...
        if np.sum(schedule[:,day+1]) != -30:
            ...
            if stopdate == datetime.datetime.now():
                if model_type == 'nbawalkod':
                    df.to_excel('predictions/predictions.xls', header = True,index=False,sheet_name ='nbawalkod')

                if model_type == 'nba_gat':
                    df.to_excel('predictions/predictions.xls', header = True,index=False,sheet_name = 'nba_gat')

                if model_type == 'nba_ARMA':
                    df.to_excel('predictions/predictions.xls', header = True,index=False,sheet_name = 'nba_ARMA')

                if model_type == 'nba_gin':
                    df.to_excel('predictions/predictions.xls', header = True,index=False,sheet_name = 'nba_gin')
